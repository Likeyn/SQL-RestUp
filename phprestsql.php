...
class PHPRestSQL {
    ...
    function PHPRestSQL($iniFile = 'phprestsql.ini') {
        ...
        if (isset($_SERVER['REQUEST_URI']) && isset($_SERVER['REQUEST_METHOD'])) {
            ...
            $urlString = substr($_SERVER['REQUEST_URI'], strlen($this->config['settings']['baseURL']));
				if (!empty($_SERVER['QUERY_STRING'])) { $urlString = substr($urlString, 0, -1 - strlen($_SERVER['QUERY_STRING'])); }	#PoufTest
				...
				if (isset($urlParts[0]) && $urlParts[0] == '') { array_shift($urlParts); }
				
				//Allows to force the use of tables that match the giving prefix, if set in phprestsql.ini #PoufTest
				if (isset($this->config['database']['tablePrefix'])) {
					$this->tablePrefix = $this->config['database']['tablePrefix'];
					if (preg_match('/^'.$this->tablePrefix.'/', $urlParts[0])) { $this->table = $urlParts[0]; }
					else { $this->table = FALSE; }
				} //Allows to force the only table to use, if set in phprestsql.ini #PoufTest
				else if (isset($this->config['database']['table'])) {
					$this->table = $this->config['database']['table'];
					if (isset($urlParts[0]) && $urlParts[0] != $this->table) { array_unshift($urlParts, $this->table); }
				}
				else if (!empty($urlParts[0])) { $this->table = $urlParts[0]; }
    ...
    function get() {
        if ($this->table) {
            ...
            if ($primary) {
                if (!empty($this->uid) && count($primary) == count($this->uid)) { // get a row by id
                    $this->display = 'row';
                    $where = '';
                    foreach($primary as $key => $pri) {
                        $where .= $pri.' = \''.$this->db->escape($this->uid[$key]).'\' AND ';
                    }
                    ...
                    if ($resource) {
                            ...
                            while ($row = $this->db->row($resource)) {
                                ...
                                foreach ($row as $column => $data) {
                                    ...
                                    if (substr($column, -strlen($this->config['database']['foreignKeyPostfix'])) == $this->config['database']['foreignKeyPostfix']) {
													$prefix = $this->config['database']['foreignKeyPrefix'] or '';
													$field['xlink'] = $this->config['settings']['baseURL'].'/'.$prefix.substr($column, 0, -strlen($this->config['database']['foreignKeyPostfix'])).'/'.$data;
                                    }
                                    $values[] = $field;
                                }
					 ...
					 else if (!empty($_SERVER['QUERY_STRING'])) { //get rows by search
						  $this->display = 'table';
                    $where = '';
						  
						  //Detect logical operator
						  if (isset($_GET['or'])) { $logop = ' OR '; }
						  else { $logop = ' AND '; }
						  
						  //Detect condition operator
						  #if (isset($_GET['in'])) { $condop = 'IN'; }
						  #else { $condop = 'BETWEEN'; }
						  
						  //Filter GET vars & construct 'where' for SQL request
						  $searchQuery = filter_input_array(INPUT_GET, FILTER_SANITIZE_STRING);
						  if (array_key_exists('or', $searchQuery)) { unset($searchQuery['or']); }
						  if (array_key_exists('in', $searchQuery)) { unset($searchQuery['in']); }
						  
						  $string = array();
						  foreach ($searchQuery as $key => $val) {
							  if (!is_array($val)) {
									$string[] = '`'.$this->db->escape($key).'` LIKE "'.$this->db->escape($val).'"';
							  }
							  else {
									//Detect condition operator (used with arrays vars)
									if (isset($_GET['in'])) {
										$val = implode(', ', $val);
										$string[] = '`'.$this->db->escape($key).'` IN ('.$this->db->escape($val).')';
									}
									else {
										$val = implode(' AND ', $val);
										$string[] = '`'.$this->db->escape($key).'` BETWEEN '.$this->db->escape($val);
									}
							  }
						  }
						  $where = implode($logop, $string);
                    ...
                    if ($resource) {
                        if ($this->db->numRows($resource) > 0) {
                            while ($row = $this->db->row($resource)) {
										  $primOutput = array();
										  foreach ($primary as $priKey) { $primOutput[] = $row[$priKey]; }
                                $this->output['table'][] = array(
                                    'xlink' => $this->config['settings']['baseURL'].'/'.$this->table.'/'.join("/", $primOutput),
                                    'value' => join("/", $primOutput)
                                );
                            }
                        }
                        $this->generateResponseData();
            ...
            if ($resource) {
                if ($this->db->numRows($resource) > 0) {
                    while ($row = $this->db->row($resource)) {
								//Restrict display to matching prefix tables only #PoufTest
								if (isset($this->tablePrefix) && !preg_match('/^'.$this->tablePrefix.'/', reset($row))) { continue; }
...
/**
 * Tests if $value is a SQL search operator
 * @param string $value The value to test
 * @return mixed 1 if match, 0 if no match, FALSE if problem
 */
